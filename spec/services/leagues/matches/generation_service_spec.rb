...
describe Leagues::Matches::GenerationService do
  describe 'single elimination' do
    it 'works' do
      ...
      map = create(:map)
      match_options = {
        round_number: 0, rounds_attributes: [
          { map: map }
        ]
      }
      match = described_class.call(division, match_options, :single_elimination, round: 0)
      ...
      expect(match1.away_team).to eq(team1)
      expect(match1.has_winner).to be true
      expect(match1.round_number).to eq(0)
      ...
      expect(match2.away_team).to eq(team4)
      expect(match2.has_winner).to be true
      expect(match2.round_number).to eq(0)
      expect(match2.rounds.size).to eq(1)
      expect(match2.rounds.first.map).to eq(map)

      match1.update!(
        status: :confirmed, rounds_attributes: [
          { id: match1.rounds.first.id, home_team_score: 1, away_team_score: 2 }
        ]
      )
      expect(match1.winner).to eq(team1)

      match2.update!(
        status: :confirmed, rounds_attributes: [
          { id: match2.rounds.first.id, home_team_score: 6, away_team_score: 3 }
        ]
      )
      expect(match2.winner).to eq(team2)

      rosters = division.rosters.ordered(league).to_a
      expect(rosters).to eq([team2, team1, team3, team4])
      ...
      expect(division.matches.round(1).size).to eq(1)
      match = division.matches.round(1).first
      ...
      match.update!(
        status: :confirmed, rounds_attributes: [
          { id: match.rounds.first.id, home_team_score: 3, away_team_score: 2 }
        ]
      )
      expect(match.winner).to eq(team1)

      rosters = division.rosters.ordered(league).to_a
      expect(rosters).to eq([team1, team2, team3, team4])
    end
    ...
    it 'handles large amount of teams' do
      ...
      (0...5).each do |round|
        ...
        invalid = described_class.call(division, match_options, :single_elimination, round: round)
